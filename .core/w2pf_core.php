...
class w2pf_core_pressExpress {
	...
	*/
	function load_languages () {
		if( $this->Config->read('use.i18n') ) {
			load_plugin_textdomain('pressbackup', false, $this->Path->Dir['LANG'] );
		}
	}
	...
	*/
	function import ($name) {
		...
		if(file_exists($file = $this->Path->Dir['LIB'] . DS . $name) ) {		
			require_once ($this->Path->Dir['LIB'] . DS . $name);
		} else {
			$this->import('misc.php');
			$this->import('Pro.php');
